...
class salaire 
{
	...
	public function liste_poste($id_user)
	{
		$select = DataBase::connect()->query("select * from poste as p inner join salaire as s on p.id_poste=s.id_poste where p.id_user='$id_user' ORDER BY p.id_poste DESC");
		
		while($donnees = $select->fetch(PDO::FETCH_OBJ))
		{
			$id_poste= $donnees->id_poste;
			$id_salaire= $donnees->id_salaire;
			$poste= $donnees->poste;
			...
			echo "</td>";
			echo "<td>";
			echo "<a href='modifier_salaire.php?id=$id_user&id_c=$id_salaire'>"; 
